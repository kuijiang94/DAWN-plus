...
opt = Config('training_DAWN_PLUS.yml')
...
file1 = 'DAWN_PLUS_PAR.txt'
file2 = 'DAWN_PLUS_PSNR.txt'
file3 = 'DAWN_PLUS_LOSS.txt'
######### Set Seeds ###########
random.seed(1234)
np.random.seed(1234)
...
if opt.TRAINING.RESUME:
    ...
    path_chk_rest    = utils.get_last_path(model_dir, '_epoch_585.pth')
    # path_chk_rest    = utils.get_last_path(model_dir, '_epoch_290.pth')
    ...
    for i in range(1, start_epoch):
        scheduler.step()
    new_lr = scheduler.get_lr()[0] * 1.25
    print('------------------------------------------------------------------------------')
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        #loss = (0.5*loss_char) + (0.2*loss_edge) - (0.15*loss_SSIM)# + 0.05*l_pix# + (0.1*loss_char_fea)# + 0.1*loss_char_V + 0.03*loss_char_H# 0.15,0.2
        loss = (0.5*loss_char) + (0.2*loss_edge) - (0.15*loss_SSIM)
